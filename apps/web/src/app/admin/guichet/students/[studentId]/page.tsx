...
export default async function AdminStudentDetailPage({
  ...
  try {
    ...
    if (pgError?.code === 'PGRST116') notFound()
    throw error
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/admin/guichet/students" className="text-sm text-muted-foreground hover:underline">
          ← Étudiants
        </Link>
      ...
      <div className="flex gap-2">
        <Button asChild variant="outline" size="sm">
          <Link href={`/admin/guichet/students/${studentId}/print-qr`}>Imprimer QR</Link>
        </Button>
      </div>
      ...
      <div className="border rounded-md p-4 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Button asChild size="sm">
            <Link href={`/admin/guichet/students/${studentId}/subscriptions/new`}>
              Vendre abonnement
            </Link>
          </Button>
        </div>
