...
export function SellSubscriptionForm({ studentId, plans, stackStartDate }: Props) {
  ...
  const { execute, status } = useAction(createSubscriptionAction, {
    onSuccess: ({ data }) => {
      ...
      router.push(`/admin/guichet/students/${studentId}`)
    },
    ...
    <div className="space-y-4 max-w-sm">
      ...
      {previewEndDate && (
        <p className="text-sm text-muted-foreground">
          Fin prévue : <span className="font-medium text-foreground">{previewEndDate}</span>
        </p>
      )}

      <Button
        disabled={!selectedPlanId || status === 'executing'}
        onClick={() => execute({ student_id: studentId, plan_id: selectedPlanId, start_date: stackStartDate })}
      >
