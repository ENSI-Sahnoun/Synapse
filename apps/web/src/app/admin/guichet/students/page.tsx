...
export const metadata = { title: 'Guichet étudiants — Synapse' }

export default async function AdminGuichetStudentsPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Guichet étudiants</h1>
        <Button asChild>
          <Link href="/admin/guichet/students/new">Nouvel étudiant</Link>
        </Button>
      </div>
            ...
            {students.map((s) => (
              <tr key={s.id} className="border-b last:border-0">
                ...
                <td className="px-4 py-2">
                  <Link href={`/admin/guichet/students/${s.id}`} className="text-primary hover:underline text-xs">
                    Voir
                  </Link>
                </td>
              </tr>
