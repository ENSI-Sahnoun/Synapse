...
export function ChoiceCards() {
  return (
    // Solid cream-200 (not cream-100) — a plain overlay tint here kept reading as a flat
    // white seam against the dark hero; a real background-color change can't be missed.
    <section id="choix" className="relative overflow-hidden bg-[var(--synapse-cream-200)] py-24 sm:py-32">
      <Backdrop variant="a" />
      <FloatingShapes />
      ...
      <div className="mx-auto grid max-w-3xl gap-4 md:grid-cols-2">
        <TiltCard
          ...
          desc="Réservez votre place, scannez votre QR à l’arrivée, suivez votre abonnement et vos points fidélité."
        ...
        <TiltCard
          ...
          desc="Freelance, indépendant ou équipe ? Un accès dédié, des horaires étendus, un plan pensé pour votre rythme."
  ...
  return (
    ...
    >
      <Link
        ...
        className="group relative block h-full overflow-hidden rounded-[1.5rem] p-5 outline-none transition-shadow duration-300 focus-visible:ring-2 focus-visible:ring-[var(--synapse-orange-400)] focus-visible:ring-offset-2 sm:p-6"
      ...
      >
        ...
        {!reduce && (
          <motion.span
            aria-hidden
            className="absolute inset-0 rounded-[1.5rem] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="h-9 w-16 shrink-0">{motif}</div>
          </div>
          ...
          <h3
            className="mt-1 text-[1.3rem] font-normal leading-tight tracking-[-0.01em]"
            style={{ fontFamily: '"DM Serif Display", Georgia, serif' }}
          >
            {title}
          </h3>
          <p className={`mt-1.5 max-w-sm text-pretty text-sm leading-relaxed ${isWarm ? 'text-[var(--synapse-stone-600)]' : 'text-white/65'}`}>
            {desc}
          </p>

          <ul className="mt-3 flex flex-col gap-1.5">
            {chips.map((c) => (
              <li key={c} className="flex items-center gap-2 text-[0.85rem] font-medium">
                <Check
                  className={`size-3.5 shrink-0 ${isWarm ? 'text-[var(--synapse-brown-500)]' : 'text-[var(--synapse-orange-400)]'}`}
                  strokeWidth={2.5}
                />
...
/** Student motif: a compact rocket badge with a soft breathing glow. */
function StudentMotif() {
  const reduce = useReducedMotion();
  return (
    <motion.div
      className="flex h-full items-center justify-center rounded-lg bg-[var(--synapse-cream-200)] ring-1 ring-[var(--synapse-cream-300)]"
    ...
    >
      <motion.span
        ...
        transition={{ duration: 2.6, repeat: Infinity, ease: 'easeInOut' }}
      >
        <Rocket className="size-5 text-[var(--synapse-brown-600)]" />
      </motion.span>
    </motion.div>
  );
}
  ...
  return (
    <div className="flex h-full flex-col justify-center gap-1">
      {widths.map((w, i) => (
        <motion.span
          key={i}
          className="h-1.5 rounded-full bg-[var(--synapse-orange-500)]"
